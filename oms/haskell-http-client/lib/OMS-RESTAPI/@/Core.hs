...
newConfig = do
    ...
    return $ @Config
        ...
        , configUserAgent = "oms---rest-api-...@/0.1.0.0"
...
addAuthMethod :: AuthMethod auth => @Config -> auth -> @Config
addAuthMethod config@@Config {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: @Config -> IO @Config
...
withStderrLogging :: @Config -> IO @Config
...
data @Request req contentType res accept = @Request
  { rMethod  :: NH.Method   -- ^ Method of @Request
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of @Request
  , rParams   :: Params -- ^ params of @Request
...
_mkRequest :: NH.Method -- ^ Method 
          ...
          -> @Request req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = @Request m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader :: @Request req contentType res accept -> [NH.Header] -> @Request req contentType res accept
...
setQuery :: @Request req contentType res accept -> [NH.QueryItem] -> @Request req contentType res accept
setQuery req query = 
  req &
  L.over
...
_applyAuthMethods
  ...
  -> @Config
  -> IO (@Request req contentType res accept)
_applyAuthMethods req config@(@Config {configAuthMethods = as}) =
  foldlM go req as
  where
