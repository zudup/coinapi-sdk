...
```php
chainsChainIdDappsSushiswapLiquidityPositionHistoricalGet($chain_id, $start_block, $end_block, $start_date, $end_date, $pool_id)
```
...
try {
    $apiInstance->chainsChainIdDappsSushiswapLiquidityPositionHistoricalGet($chain_id, $start_block, $end_block, $start_date, $end_date, $pool_id);
} catch (Exception $e) {
    echo 'Exception when calling LiquidityPositionApi->chainsChainIdDappsSushiswapLiquidityPositionHistoricalGet: ', $e->getMessage(), PHP_EOL;
}
```
