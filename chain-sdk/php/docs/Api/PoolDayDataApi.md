...
```php
chainsChainIdDappsSushiswapPoolDayDataHistoricalGet($chain_id, $start_block, $end_block, $start_date, $end_date, $pool_id)
```
...
try {
    $apiInstance->chainsChainIdDappsSushiswapPoolDayDataHistoricalGet($chain_id, $start_block, $end_block, $start_date, $end_date, $pool_id);
} catch (Exception $e) {
    echo 'Exception when calling PoolDayDataApi->chainsChainIdDappsSushiswapPoolDayDataHistoricalGet: ', $e->getMessage(), PHP_EOL;
}
```
