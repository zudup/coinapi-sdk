...
```php
dappsCowPoiHistoricalGet($start_block, $end_block, $start_date, $end_date)
```
...
try {
    $apiInstance->dappsCowPoiHistoricalGet($start_block, $end_block, $start_date, $end_date);
} catch (Exception $e) {
    echo 'Exception when calling CowApi->dappsCowPoiHistoricalGet: ', $e->getMessage(), PHP_EOL;
}
```
