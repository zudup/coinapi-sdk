...
```php
chainsChainIdDappsSushiswapTransactionsHistoricalGet($chain_id, $start_block, $end_block, $start_date, $end_date)
```
...
try {
    $apiInstance->chainsChainIdDappsSushiswapTransactionsHistoricalGet($chain_id, $start_block, $end_block, $start_date, $end_date);
} catch (Exception $e) {
    echo 'Exception when calling TransactionsApi->chainsChainIdDappsSushiswapTransactionsHistoricalGet: ', $e->getMessage(), PHP_EOL;
}
```
