...
```php
chainsChainIdDappsSushiswapTokenDayDataHistoricalGet($chain_id, $start_block, $end_block, $start_date, $end_date, $token_id)
```
...
try {
    $apiInstance->chainsChainIdDappsSushiswapTokenDayDataHistoricalGet($chain_id, $start_block, $end_block, $start_date, $end_date, $token_id);
} catch (Exception $e) {
    echo 'Exception when calling TokenDayDataApi->chainsChainIdDappsSushiswapTokenDayDataHistoricalGet: ', $e->getMessage(), PHP_EOL;
}
```
