...
module OnChain.Core where

import OnChain.MimeTypes
import OnChain.Logging
...
-- |
data OnChainConfig = OnChainConfig
...
-- | display the config
instance P.Show OnChainConfig where
  show c =
    T.printf
...
-- | constructs a default OnChainConfig
--
-- configHost:
--
...
-- @"onchain/0.1.0.0"@
--
newConfig :: IO OnChainConfig
newConfig = do
    logCxt <- initLogContext
    return $ OnChainConfig
        { configHost = "https://onchain.coinapi.io"
        , configUserAgent = "onchain/0.1.0.0"
...
addAuthMethod :: AuthMethod auth => OnChainConfig -> auth -> OnChainConfig
addAuthMethod config@OnChainConfig {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: OnChainConfig -> IO OnChainConfig
...
withStderrLogging :: OnChainConfig -> IO OnChainConfig
...
withNoLogging :: OnChainConfig -> OnChainConfig
...
data OnChainRequest req contentType res accept = OnChainRequest
  { rMethod  :: NH.Method   -- ^ Method of OnChainRequest
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of OnChainRequest
  , rParams   :: Params -- ^ params of OnChainRequest
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (OnChainRequest req contentType res accept) NH.Method
rMethodL f OnChainRequest{..} = (\rMethod -> OnChainRequest { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (OnChainRequest req contentType res accept) [BCL.ByteString]
rUrlPathL f OnChainRequest{..} = (\rUrlPath -> OnChainRequest { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (OnChainRequest req contentType res accept) Params
rParamsL f OnChainRequest{..} = (\rParams -> OnChainRequest { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (OnChainRequest req contentType res accept) [P.TypeRep]
rAuthTypesL f OnChainRequest{..} = (\rAuthTypes -> OnChainRequest { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => OnChainRequest req contentType res accept -> param -> OnChainRequest req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: OnChainRequest req contentType res accept -> param -> OnChainRequest req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: OnChainRequest req contentType res accept -> param -> OnChainRequest req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
-- ** OnChainRequest Utils

_mkRequest :: NH.Method -- ^ Method
          -> [BCL.ByteString] -- ^ Endpoint
          -> OnChainRequest req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = OnChainRequest m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader ::
     OnChainRequest req contentType res accept
  -> [NH.Header]
  -> OnChainRequest req contentType res accept
...
removeHeader :: OnChainRequest req contentType res accept -> [NH.HeaderName] -> OnChainRequest req contentType res accept
...
_setContentTypeHeader :: forall req contentType res accept. MimeType contentType => OnChainRequest req contentType res accept -> OnChainRequest req contentType res accept
_setContentTypeHeader req =
    case mimeType (P.Proxy :: P.Proxy contentType) of
        Just m -> req `setHeader` [("content-type", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["content-type"]

_setAcceptHeader :: forall req contentType res accept. MimeType accept => OnChainRequest req contentType res accept -> OnChainRequest req contentType res accept
...
addForm :: OnChainRequest req contentType res accept -> WH.Form -> OnChainRequest req contentType res accept
addForm req newform =
...
_addMultiFormPart :: OnChainRequest req contentType res accept -> NH.Part -> OnChainRequest req contentType res accept
...
_setBodyBS :: OnChainRequest req contentType res accept -> B.ByteString -> OnChainRequest req contentType res accept
_setBodyBS req body =
    req & L.set (rParamsL . paramsBodyL) (ParamBodyB body)

_setBodyLBS :: OnChainRequest req contentType res accept -> BL.ByteString -> OnChainRequest req contentType res accept
_setBodyLBS req body =
    req & L.set (rParamsL . paramsBodyL) (ParamBodyBL body)

_hasAuthType :: AuthMethod authMethod => OnChainRequest req contentType res accept -> P.Proxy authMethod -> OnChainRequest req contentType res accept
...
class P.Typeable a =>
  ...
  applyAuthMethod
    ...
    -> a
    -> OnChainRequest req contentType res accept
    -> IO (OnChainRequest req contentType res accept)
...
_applyAuthMethods
  ...
  -> OnChainConfig
  -> IO (OnChainRequest req contentType res accept)
_applyAuthMethods req config@(OnChainConfig {configAuthMethods = as}) =
  foldlM go req as
  where
