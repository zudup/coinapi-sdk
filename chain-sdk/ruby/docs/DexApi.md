...
> dapps_dex_poi_historical_get(opts)
...
begin
  ...
  api_instance.dapps_dex_poi_historical_get(opts)
rescue OpenapiClient::ApiError => e
  puts "Error when calling DexApi->dapps_dex_poi_historical_get: #{e}"
end
```

#### Using the dapps_dex_poi_historical_get_with_http_info variant

This returns an Array which contains the response data (`nil` in this case), status code and headers.

> <Array(nil, Integer, Hash)> dapps_dex_poi_historical_get_with_http_info(opts)

```ruby
begin
  
  data, status_code, headers = api_instance.dapps_dex_poi_historical_get_with_http_info(opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => nil
rescue OpenapiClient::ApiError => e
  puts "Error when calling DexApi->dapps_dex_poi_historical_get_with_http_info: #{e}"
end
```
