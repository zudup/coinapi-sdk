...
> chains_chain_id_dapps_sushiswap_transactions_historical_get(chain_id, opts)
...
begin
  ...
  api_instance.chains_chain_id_dapps_sushiswap_transactions_historical_get(chain_id, opts)
rescue OpenapiClient::ApiError => e
  puts "Error when calling TransactionsApi->chains_chain_id_dapps_sushiswap_transactions_historical_get: #{e}"
end
```

#### Using the chains_chain_id_dapps_sushiswap_transactions_historical_get_with_http_info variant

This returns an Array which contains the response data (`nil` in this case), status code and headers.

> <Array(nil, Integer, Hash)> chains_chain_id_dapps_sushiswap_transactions_historical_get_with_http_info(chain_id, opts)

```ruby
begin
  
  data, status_code, headers = api_instance.chains_chain_id_dapps_sushiswap_transactions_historical_get_with_http_info(chain_id, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => nil
rescue OpenapiClient::ApiError => e
  puts "Error when calling TransactionsApi->chains_chain_id_dapps_sushiswap_transactions_historical_get_with_http_info: #{e}"
end
```
