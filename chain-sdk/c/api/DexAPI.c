...
DexAPI_dexGetOrdersHistorical(apiClient_t *apiClient, long startBlock , long endBlock , char startDate , char endDate , char * id , char * buy_token , char * sell_token )
{
    ...
    // query parameters
    char *keyQuery_buy_token = NULL;
    char * valueQuery_buy_token = NULL;
    keyValuePair_t *keyPairQuery_buy_token = 0;
    if (buy_token)
    {
        keyQuery_buy_token = strdup("buy_token");
        valueQuery_buy_token = strdup((buy_token));
        keyPairQuery_buy_token = keyValuePair_create(keyQuery_buy_token, valueQuery_buy_token);
        list_addElement(localVarQueryParameters,keyPairQuery_buy_token);
    }

    // query parameters
    char *keyQuery_sell_token = NULL;
    char * valueQuery_sell_token = NULL;
    keyValuePair_t *keyPairQuery_sell_token = 0;
    if (sell_token)
    {
        keyQuery_sell_token = strdup("sell_token");
        valueQuery_sell_token = strdup((sell_token));
        keyPairQuery_sell_token = keyValuePair_create(keyQuery_sell_token, valueQuery_sell_token);
        list_addElement(localVarQueryParameters,keyPairQuery_sell_token);
    }
    list_addElement(localVarHeaderType,"text/plain"); //produces
    list_addElement(localVarHeaderType,"application/json"); //produces
    list_addElement(localVarHeaderType,"text/json"); //produces
    ...
    if(keyQuery_buy_token){
        free(keyQuery_buy_token);
        keyQuery_buy_token = NULL;
    }
    if(valueQuery_buy_token){
        free(valueQuery_buy_token);
        valueQuery_buy_token = NULL;
    }
    if(keyPairQuery_buy_token){
        keyValuePair_free(keyPairQuery_buy_token);
        keyPairQuery_buy_token = NULL;
    }
    if(keyQuery_sell_token){
        free(keyQuery_sell_token);
        keyQuery_sell_token = NULL;
    }
    if(valueQuery_sell_token){
        free(valueQuery_sell_token);
        valueQuery_sell_token = NULL;
    }
    if(keyPairQuery_sell_token){
        keyValuePair_free(keyPairQuery_sell_token);
        keyPairQuery_sell_token = NULL;
    }
    return elementToReturn;
end:
    free(localVarPath);
...
DexAPI_dexGetWithdrawRequestsHistorical(apiClient_t *apiClient, long startBlock , long endBlock , char startDate , char endDate , char * id , char * user )
{
    ...
    // create the path
    long sizeOfPath = strlen("/dapps/dex/withdrawrequests/historical")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/dapps/dex/withdrawrequests/historical");
