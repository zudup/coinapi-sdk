...
class LiquidityPositionSnapshotsApi {
  LiquidityPositionSnapshotsApi([ApiClient? apiClient]) : apiClient = apiClient ?? defaultApiClient;
  ...
  ///
  /// * [String] poolId:
  Future<Response> chainsChainIdDappsUniswapv2LiquidityPositionSnapshotsHistoricalGetWithHttpInfo(String chainId, { int? startBlock, int? endBlock, DateTime? startDate, DateTime? endDate, String? poolId, }) async {
    // ignore: prefer_const_declarations
    final path = r'/chains/{chain_id}/dapps/uniswapv2/liquidityPositionSnapshots/historical'
      .replaceAll('{chain_id}', chainId);
  ...
  ///
  /// * [String] poolId:
  Future<void> chainsChainIdDappsUniswapv2LiquidityPositionSnapshotsHistoricalGet(String chainId, { int? startBlock, int? endBlock, DateTime? startDate, DateTime? endDate, String? poolId, }) async {
    final response = await chainsChainIdDappsUniswapv2LiquidityPositionSnapshotsHistoricalGetWithHttpInfo(chainId,  startBlock: startBlock, endBlock: endBlock, startDate: startDate, endDate: endDate, poolId: poolId, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }
