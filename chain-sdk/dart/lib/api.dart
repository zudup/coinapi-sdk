//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

library openapi.api;

import 'dart:async';
import 'dart:convert';
import 'dart:io';

import 'package:http/http.dart';
import 'package:intl/intl.dart';
import 'package:meta/meta.dart';

part 'api_client.dart';
part 'api_helper.dart';
part 'api_exception.dart';
part 'auth/authentication.dart';
part 'auth/api_key_auth.dart';
part 'auth/oauth.dart';
part 'auth/http_basic_auth.dart';
part 'auth/http_bearer_auth.dart';

part 'api/accounts_api.dart';
part 'api/add_liquidity_event_api.dart';
part 'api/admin_fee_change_log_api.dart';
part 'api/amplification_coeff_change_log_api.dart';
part 'api/batch_api.dart';
part 'api/bundles_api.dart';
part 'api/burns_api.dart';
part 'api/coins_api.dart';
part 'api/contracts_api.dart';
part 'api/contracts_version_api.dart';
part 'api/daily_volume_api.dart';
part 'api/day_data_api.dart';
part 'api/deposit_api.dart';
part 'api/factory_api.dart';
part 'api/fee_change_log_api.dart';
part 'api/gauge_api.dart';
part 'api/gauge_deposit_api.dart';
part 'api/gauge_liquidity_api.dart';
part 'api/gauge_total_weight_api.dart';
part 'api/gauge_type_api.dart';
part 'api/gauge_type_weight_api.dart';
part 'api/gauge_weight_api.dart';
part 'api/gauge_weight_vote_api.dart';
part 'api/gauge_withdraw_api.dart';
part 'api/hour_data_api.dart';
part 'api/hourly_volume_api.dart';
part 'api/liquidity_position_api.dart';
part 'api/liquidity_position_snapshots_api.dart';
part 'api/lp_token_api.dart';
part 'api/mints_api.dart';
part 'api/orders_api.dart';
part 'api/poi_api.dart';
part 'api/pool_day_data_api.dart';
part 'api/pool_hour_data_api.dart';
part 'api/pools_api.dart';
part 'api/position_snapshot_api.dart';
part 'api/positions_api.dart';
part 'api/prices_api.dart';
part 'api/proposals_api.dart';
part 'api/proposals_vote_api.dart';
part 'api/remove_liquidity_event_api.dart';
part 'api/remove_liquidity_one_event_api.dart';
part 'api/settlement_api.dart';
part 'api/solution_api.dart';
part 'api/stats_api.dart';
part 'api/sushiswap_api.dart';
part 'api/swaps_api.dart';
part 'api/system_state_api.dart';
part 'api/tick_day_data_api.dart';
part 'api/ticks_api.dart';
part 'api/token_day_data_api.dart';
part 'api/token_hour_data_api.dart';
part 'api/tokens_api.dart';
part 'api/trades_api.dart';
part 'api/transactions_api.dart';
part 'api/transfer_ownership_event_api.dart';
part 'api/underlying_coin_api.dart';
part 'api/uniswap_day_data_api.dart';
part 'api/uniswap_v2_api.dart';
part 'api/uniswap_v3_api.dart';
part 'api/users_api.dart';
part 'api/voting_app_api.dart';
part 'api/weekly_volume_api.dart';
part 'api/withdraw_api.dart';
part 'api/withdraw_request_api.dart';

part 'model/big_integer.dart';
part 'model/bundle_v3_dto.dart';
part 'model/burn_v3_dto.dart';
part 'model/e_trade_aggressive_side.dart';
part 'model/factory_v3_dto.dart';
part 'model/mint_v3_dto.dart';
part 'model/pair_dto.dart';
part 'model/pair_v2_dto.dart';
part 'model/pool_day_data_v3_dto.dart';
part 'model/pool_hour_data_v3_dto.dart';
part 'model/pool_v3_dto.dart';
part 'model/position_snapshot_v3_dto.dart';
part 'model/position_v3_dto.dart';
part 'model/swap_dto.dart';
part 'model/swap_v2_dto.dart';
part 'model/swap_v3_dto.dart';
part 'model/tick_day_data_v3_dto.dart';
part 'model/tick_v3_dto.dart';
part 'model/token_dto.dart';
part 'model/token_hour_data_v3_dto.dart';
part 'model/token_v2_dto.dart';
part 'model/token_v3_dto.dart';
part 'model/token_v3_day_data_dto.dart';
part 'model/uniswap_day_data_v3_dto.dart';


const _delimiters = {'csv': ',', 'ssv': ' ', 'tsv': '\t', 'pipes': '|'};
const _dateEpochMarker = 'epoch';
final _dateFormatter = DateFormat('yyyy-MM-dd');
final _regList = RegExp(r'^List<(.*)>$');
final _regSet = RegExp(r'^Set<(.*)>$');
final _regMap = RegExp(r'^Map<String,(.*)>$');

ApiClient defaultApiClient = ApiClient();
