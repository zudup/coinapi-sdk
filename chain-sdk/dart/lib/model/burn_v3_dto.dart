...
class BurnV3DTO {
  /// Returns a new [BurnV3DTO] instance.
  BurnV3DTO({
    this.entryTime,
    this.recvTime,
    this.blockNumber,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is BurnV3DTO &&
  ...
  @override
  String toString() => 'BurnV3DTO[entryTime=$entryTime, recvTime=$recvTime, blockNumber=$blockNumber, id=$id, transaction=$transaction, pool=$pool, token0=$token0, token1=$token1, timestamp=$timestamp, owner=$owner, origin=$origin, amount=$amount, amount0=$amount0, amount1=$amount1, amountUsd=$amountUsd, tickLower=$tickLower, tickUpper=$tickUpper, logIndex=$logIndex, vid=$vid]';
  ...
  /// Returns a new [BurnV3DTO] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static BurnV3DTO? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "BurnV3DTO[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "BurnV3DTO[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<BurnV3DTO>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <BurnV3DTO>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = BurnV3DTO.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, BurnV3DTO> mapFromJson(dynamic json) {
    final map = <String, BurnV3DTO>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = BurnV3DTO.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of BurnV3DTO-objects as value to a dart map
  static Map<String, List<BurnV3DTO>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<BurnV3DTO>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = BurnV3DTO.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
