...
class TickDayDataV3DTO {
  /// Returns a new [TickDayDataV3DTO] instance.
  TickDayDataV3DTO({
    this.entryTime,
    this.recvTime,
    this.blockNumber,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is TickDayDataV3DTO &&
  ...
  @override
  String toString() => 'TickDayDataV3DTO[entryTime=$entryTime, recvTime=$recvTime, blockNumber=$blockNumber, id=$id, date=$date, pool=$pool, tick=$tick, liquidityGross=$liquidityGross, liquidityNet=$liquidityNet, volumeToken0=$volumeToken0, volumeToken1=$volumeToken1, volumeUsd=$volumeUsd, feesUsd=$feesUsd, feeGrowthOutside0x128=$feeGrowthOutside0x128, feeGrowthOutside1x128=$feeGrowthOutside1x128, vid=$vid]';
  ...
  /// Returns a new [TickDayDataV3DTO] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static TickDayDataV3DTO? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "TickDayDataV3DTO[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "TickDayDataV3DTO[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<TickDayDataV3DTO>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <TickDayDataV3DTO>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = TickDayDataV3DTO.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, TickDayDataV3DTO> mapFromJson(dynamic json) {
    final map = <String, TickDayDataV3DTO>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = TickDayDataV3DTO.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of TickDayDataV3DTO-objects as value to a dart map
  static Map<String, List<TickDayDataV3DTO>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<TickDayDataV3DTO>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = TickDayDataV3DTO.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
