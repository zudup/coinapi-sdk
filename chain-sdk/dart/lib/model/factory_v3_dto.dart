...
class FactoryV3DTO {
  /// Returns a new [FactoryV3DTO] instance.
  FactoryV3DTO({
    this.entryTime,
    this.recvTime,
    this.blockNumber,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is FactoryV3DTO &&
  ...
  /// Returns a new [FactoryV3DTO] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static FactoryV3DTO? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "FactoryV3DTO[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "FactoryV3DTO[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<FactoryV3DTO>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <FactoryV3DTO>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = FactoryV3DTO.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, FactoryV3DTO> mapFromJson(dynamic json) {
    final map = <String, FactoryV3DTO>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = FactoryV3DTO.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of FactoryV3DTO-objects as value to a dart map
  static Map<String, List<FactoryV3DTO>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<FactoryV3DTO>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = FactoryV3DTO.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
