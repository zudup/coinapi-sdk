//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for CurvePoolDTO
void main() {
  // final instance = CurvePoolDTO();

  group('test CurvePoolDTO', () {
    // DateTime entryTime
    test('to test the property `entryTime`', () async {
      // TODO
    });

    // DateTime recvTime
    test('to test the property `recvTime`', () async {
      // TODO
    });

    // Number of block in which entity was recorded.
    // int blockNumber
    test('to test the property `blockNumber`', () async {
      // TODO
    });

    // Pool address.
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // Pool's human-readable name.
    // String name
    test('to test the property `name`', () async {
      // TODO
    });

    // Identify whether pool is a metapool.
    // bool isMeta
    test('to test the property `isMeta`', () async {
      // TODO
    });

    // Registry contract address from where this pool was registered.
    // String registryAddress
    test('to test the property `registryAddress`', () async {
      // TODO
    });

    // Swap contract address.
    // String swapAddress
    test('to test the property `swapAddress`', () async {
      // TODO
    });

    // Address of the token representing LP share.
    // String lpToken
    test('to test the property `lpToken`', () async {
      // TODO
    });

    // Number of coins composing the pool.
    // String coinCount
    test('to test the property `coinCount`', () async {
      // TODO
    });

    // Number of underlying coins composing the pool.
    // String underlyingCount
    test('to test the property `underlyingCount`', () async {
      // TODO
    });

    // Amplification coefficient multiplied by n * (n - 1).
    // String a
    test('to test the property `a`', () async {
      // TODO
    });

    // Fee to charge for exchanges.
    // String fee
    test('to test the property `fee`', () async {
      // TODO
    });

    // Admin fee is represented as a percentage of the total fee collected on a swap.
    // String adminFee
    test('to test the property `adminFee`', () async {
      // TODO
    });

    // Admin address.
    // String owner
    test('to test the property `owner`', () async {
      // TODO
    });

    // Average dollar value of pool token.
    // String virtualPrice
    test('to test the property `virtualPrice`', () async {
      // TODO
    });

    // 
    // String locked
    test('to test the property `locked`', () async {
      // TODO
    });

    // 
    // DateTime addedAt
    test('to test the property `addedAt`', () async {
      // TODO
    });

    // 
    // String addedAtBlock
    test('to test the property `addedAtBlock`', () async {
      // TODO
    });

    // 
    // String addedAtTransaction
    test('to test the property `addedAtTransaction`', () async {
      // TODO
    });

    // 
    // String removedAt
    test('to test the property `removedAt`', () async {
      // TODO
    });

    // 
    // String removedAtBlock
    test('to test the property `removedAtBlock`', () async {
      // TODO
    });

    // 
    // String removedAtTransaction
    test('to test the property `removedAtTransaction`', () async {
      // TODO
    });

    // 
    // String exchangeCount
    test('to test the property `exchangeCount`', () async {
      // TODO
    });

    // 
    // String gaugeCount
    test('to test the property `gaugeCount`', () async {
      // TODO
    });

    // 
    // int vid
    test('to test the property `vid`', () async {
      // TODO
    });

    // double evaluatedAsk
    test('to test the property `evaluatedAsk`', () async {
      // TODO
    });


  });

}
