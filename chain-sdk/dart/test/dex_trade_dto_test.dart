//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for DexTradeDTO
void main() {
  // final instance = DexTradeDTO();

  group('test DexTradeDTO', () {
    // DateTime entryTime
    test('to test the property `entryTime`', () async {
      // TODO
    });

    // DateTime recvTime
    test('to test the property `recvTime`', () async {
      // TODO
    });

    // Number of block in which entity was recorded.
    // int blockNumber
    test('to test the property `blockNumber`', () async {
      // TODO
    });

    // 
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // 
    // String order
    test('to test the property `order`', () async {
      // TODO
    });

    // 
    // String owner
    test('to test the property `owner`', () async {
      // TODO
    });

    // 
    // String sellVolume
    test('to test the property `sellVolume`', () async {
      // TODO
    });

    // 
    // String buyVolume
    test('to test the property `buyVolume`', () async {
      // TODO
    });

    // 
    // String tradeBatchId
    test('to test the property `tradeBatchId`', () async {
      // TODO
    });

    // The date of the end of the batch.
    // String tradeEpoch
    test('to test the property `tradeEpoch`', () async {
      // TODO
    });

    // 
    // String buyToken
    test('to test the property `buyToken`', () async {
      // TODO
    });

    // 
    // String sellToken
    test('to test the property `sellToken`', () async {
      // TODO
    });

    // The date where the transaction was mined.
    // String createEpoch
    test('to test the property `createEpoch`', () async {
      // TODO
    });

    // 
    // String revertEpoch
    test('to test the property `revertEpoch`', () async {
      // TODO
    });

    // 
    // String txHash
    test('to test the property `txHash`', () async {
      // TODO
    });

    // 
    // String txLogIndex
    test('to test the property `txLogIndex`', () async {
      // TODO
    });

    // 
    // int vid
    test('to test the property `vid`', () async {
      // TODO
    });

    // String poolId
    test('to test the property `poolId`', () async {
      // TODO
    });

    // String transactionId
    test('to test the property `transactionId`', () async {
      // TODO
    });

    // double evaluatedPrice
    test('to test the property `evaluatedPrice`', () async {
      // TODO
    });

    // double evaluatedAmount
    test('to test the property `evaluatedAmount`', () async {
      // TODO
    });

    // TransactionsETradeAggressiveSide evaluatedAggressor
    test('to test the property `evaluatedAggressor`', () async {
      // TODO
    });


  });

}
