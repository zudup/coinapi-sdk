...
> chainsChainIdDappsUniswapv3TickDayDataHistoricalGet(chainId, opts)
...
let apiInstance = new OnChainApi.TickDayDataApi();
...
apiInstance.chainsChainIdDappsUniswapv3TickDayDataHistoricalGet(chainId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
