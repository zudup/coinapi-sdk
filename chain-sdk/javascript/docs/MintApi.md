...
> chainsChainIdDappsUniswapv2MintHistoricalGet(chainId, opts)
...
let apiInstance = new OnChainApi.MintApi();
...
apiInstance.chainsChainIdDappsUniswapv2MintHistoricalGet(chainId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
