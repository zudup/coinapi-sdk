...
local pool_hour_data_api = {}
local pool_hour_data_api_mt = {
	__name = "pool_hour_data_api";
	__index = pool_hour_data_api;
}

local function new_pool_hour_data_api(authority, basePath, schemes)
	local schemes_map = {}
...
function pool_hour_data_api:chains_chain_id_dapps_sushiswap_pool_hour_data_historical_get(chain_id, start_block, end_block, start_date, end_date, pool_id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/chains/%s/dapps/sushiswap/poolHourData/historical?startBlock=%s&endBlock=%s&startDate=%s&endDate=%s&poolId=%s",
...
function pool_hour_data_api:chains_chain_id_dapps_uniswapv3_pool_hour_data_historical_get(chain_id, start_block, end_block, start_date, end_date, pool_id)
	local req = http_request.new_from_uri({
		...
		host = self.host;
		port = self.port;
		path = string.format("%s/chains/%s/dapps/uniswapv3/poolHourData/historical?startBlock=%s&endBlock=%s&startDate=%s&endDate=%s&poolId=%s",
			self.basePath, chain_id, http_util.encodeURIComponent(start_block), http_util.encodeURIComponent(end_block), http_util.encodeURIComponent(start_date), http_util.encodeURIComponent(end_date), http_util.encodeURIComponent(pool_id));
	})
