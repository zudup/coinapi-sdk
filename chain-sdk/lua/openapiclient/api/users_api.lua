...
function users_api:chains_chain_id_dapps_dex_users_historical_get(chain_id, start_block, end_block, start_date, end_date)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/chains/%s/dapps/dex/users/historical?startBlock=%s&endBlock=%s&startDate=%s&endDate=%s",
			self.basePath, chain_id, http_util.encodeURIComponent(start_block), http_util.encodeURIComponent(end_block), http_util.encodeURIComponent(start_date), http_util.encodeURIComponent(end_date));
	})
