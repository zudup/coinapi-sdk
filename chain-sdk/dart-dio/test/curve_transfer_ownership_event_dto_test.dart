import 'package:test/test.dart';
import 'package:openapi/openapi.dart';

// tests for CurveTransferOwnershipEventDTO
void main() {
  final instance = CurveTransferOwnershipEventDTOBuilder();
  // TODO add properties to the builder and call build()

  group(CurveTransferOwnershipEventDTO, () {
    // DateTime entryTime
    test('to test the property `entryTime`', () async {
      // TODO
    });

    // DateTime recvTime
    test('to test the property `recvTime`', () async {
      // TODO
    });

    // Number of block in which entity was recorded.
    // int blockNumber
    test('to test the property `blockNumber`', () async {
      // TODO
    });

    // 
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // 
    // String pool
    test('to test the property `pool`', () async {
      // TODO
    });

    // 
    // String newAdmin
    test('to test the property `newAdmin`', () async {
      // TODO
    });

    // 
    // String block
    test('to test the property `block`', () async {
      // TODO
    });

    // 
    // String timestamp
    test('to test the property `timestamp`', () async {
      // TODO
    });

    // 
    // String transaction
    test('to test the property `transaction`', () async {
      // TODO
    });

    // 
    // int vid
    test('to test the property `vid`', () async {
      // TODO
    });

  });
}
