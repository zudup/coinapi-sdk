...
> ChainsChainIdDappsSushiswapPoolHourDataHistoricalGet(ctx, chainId).StartBlock(startBlock).EndBlock(endBlock).StartDate(startDate).EndDate(endDate).PoolId(poolId).Execute()
...
func main() {
    ...
    resp, r, err := apiClient.PoolHourDataApi.ChainsChainIdDappsSushiswapPoolHourDataHistoricalGet(context.Background(), chainId).StartBlock(startBlock).EndBlock(endBlock).StartDate(startDate).EndDate(endDate).PoolId(poolId).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `PoolHourDataApi.ChainsChainIdDappsSushiswapPoolHourDataHistoricalGet``: %v\n", err)
...
Other parameters are passed through a pointer to a apiChainsChainIdDappsSushiswapPoolHourDataHistoricalGetRequest struct via the builder pattern
