...
final api_instance = AccountApi();
final filterExchangeId = []; // List<String> | Exchange id of the specific account to provide single account instead of the list of all accounts

try {
    final result = api_instance.getAccount(filterExchangeId);
    print(result);
} catch (e) {
...
final api_instance = AccountApi();
final body = AccountData(); // AccountData | Exchange account object that needs to be add/update to the EMS
