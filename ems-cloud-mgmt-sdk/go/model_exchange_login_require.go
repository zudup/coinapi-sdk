/*
EMS - Managed Cloud REST API

This section will provide necessary information about the `CoinAPI EMS Managed Cloud REST API` protocol.  <br/><br/> This API is used to manage the overall deployment of **Execution Management System API** (`EMS API`) software,  which means that in this API, you define the accounts, credentials, and configurations for the order destinations or identify the CoinAPI endpoints where you need to connect to access the `EMS API`.  <br/><br/> Implemented Standards:   * [HTTP1.0](https://datatracker.ietf.org/doc/html/rfc1945)  * [HTTP1.1](https://datatracker.ietf.org/doc/html/rfc2616)  * [HTTP2.0](https://datatracker.ietf.org/doc/html/rfc7540)   ### Endpoints <table>   <thead>     <tr>       <th>Environment</th>       <th>Url</th>     </tr>   </thead>   <tbody>     <tr>       <td>Production</td>       <td><code>https://ems-mgmt.coinapi.io/</code></td>     </tr>     <tr>       <td>Sandbox</td>       <td><code>https://ems-mgmt-sandbox.coinapi.io/</code></td>     </tr>   </tbody> </table>  ### Authentication  To use resources that require authorized access, you will need to provide an API key to us when making HTTP requests.  There are 2 methods for passing the API key to us, you only need to use one:   1. Custom authorization header named `X-CoinAPI-Key`  2. Query string parameter named `apikey`  #### Custom authorization header  You can authorize by providing additional custom header named `X-CoinAPI-Key` and API key as its value.  Assuming that your API key is `73034021-THIS-IS-SAMPLE-KEY`, then the authorization header you should send to us will look like: <br/><br/> `X-CoinAPI-Key: 73034021-THIS-IS-SAMPLE-KEY`  <aside class=\"success\">This method is recommended by us and you should use it in production environments.</aside>  #### Query string authorization parameter  You can authorize by providing an additional parameter named `apikey` with a value equal to your API key in the query string of your HTTP request.  Assuming that your API key is `73034021-THIS-IS-SAMPLE-KEY` and that you want to request all accounts, then your query string should look like this:  <br/><br/> `GET /v1/accounts?apikey=73034021-THIS-IS-SAMPLE-KEY`  <aside class=\"notice\"> Query string method may be more practical for development activities. </aside> 

API version: v1
Contact: support@coinapi.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// ExchangeLoginRequire struct for ExchangeLoginRequire
type ExchangeLoginRequire struct {
	// Exchange identifier and optional tag identifying specific account configured when the software will be managing multiple accounts on the same exchange; for eg:  <code>BITSTAMP</code> <code>BITSTAMP/7c177641-74bd-4dbe-9b01-2497c12a5f70`</code> <code>BITSTAMP/2574</code> Allowed separators between the exchange identifier and the tag: <code>~/.,:;\\!@#$%^&*-_+=.</code> 
	ExchangeId *string `json:"exchange_id,omitempty"`
	// Location identifier
	LocationId *string `json:"location_id,omitempty"`
	// Parameters required to log into the exchange
	RequiredParameters []string `json:"required_parameters,omitempty"`
}

// NewExchangeLoginRequire instantiates a new ExchangeLoginRequire object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewExchangeLoginRequire() *ExchangeLoginRequire {
	this := ExchangeLoginRequire{}
	return &this
}

// NewExchangeLoginRequireWithDefaults instantiates a new ExchangeLoginRequire object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewExchangeLoginRequireWithDefaults() *ExchangeLoginRequire {
	this := ExchangeLoginRequire{}
	return &this
}

// GetExchangeId returns the ExchangeId field value if set, zero value otherwise.
func (o *ExchangeLoginRequire) GetExchangeId() string {
	if o == nil || o.ExchangeId == nil {
		var ret string
		return ret
	}
	return *o.ExchangeId
}

// GetExchangeIdOk returns a tuple with the ExchangeId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ExchangeLoginRequire) GetExchangeIdOk() (*string, bool) {
	if o == nil || o.ExchangeId == nil {
		return nil, false
	}
	return o.ExchangeId, true
}

// HasExchangeId returns a boolean if a field has been set.
func (o *ExchangeLoginRequire) HasExchangeId() bool {
	if o != nil && o.ExchangeId != nil {
		return true
	}

	return false
}

// SetExchangeId gets a reference to the given string and assigns it to the ExchangeId field.
func (o *ExchangeLoginRequire) SetExchangeId(v string) {
	o.ExchangeId = &v
}

// GetLocationId returns the LocationId field value if set, zero value otherwise.
func (o *ExchangeLoginRequire) GetLocationId() string {
	if o == nil || o.LocationId == nil {
		var ret string
		return ret
	}
	return *o.LocationId
}

// GetLocationIdOk returns a tuple with the LocationId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ExchangeLoginRequire) GetLocationIdOk() (*string, bool) {
	if o == nil || o.LocationId == nil {
		return nil, false
	}
	return o.LocationId, true
}

// HasLocationId returns a boolean if a field has been set.
func (o *ExchangeLoginRequire) HasLocationId() bool {
	if o != nil && o.LocationId != nil {
		return true
	}

	return false
}

// SetLocationId gets a reference to the given string and assigns it to the LocationId field.
func (o *ExchangeLoginRequire) SetLocationId(v string) {
	o.LocationId = &v
}

// GetRequiredParameters returns the RequiredParameters field value if set, zero value otherwise.
func (o *ExchangeLoginRequire) GetRequiredParameters() []string {
	if o == nil || o.RequiredParameters == nil {
		var ret []string
		return ret
	}
	return o.RequiredParameters
}

// GetRequiredParametersOk returns a tuple with the RequiredParameters field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ExchangeLoginRequire) GetRequiredParametersOk() ([]string, bool) {
	if o == nil || o.RequiredParameters == nil {
		return nil, false
	}
	return o.RequiredParameters, true
}

// HasRequiredParameters returns a boolean if a field has been set.
func (o *ExchangeLoginRequire) HasRequiredParameters() bool {
	if o != nil && o.RequiredParameters != nil {
		return true
	}

	return false
}

// SetRequiredParameters gets a reference to the given []string and assigns it to the RequiredParameters field.
func (o *ExchangeLoginRequire) SetRequiredParameters(v []string) {
	o.RequiredParameters = v
}

func (o ExchangeLoginRequire) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.ExchangeId != nil {
		toSerialize["exchange_id"] = o.ExchangeId
	}
	if o.LocationId != nil {
		toSerialize["location_id"] = o.LocationId
	}
	if o.RequiredParameters != nil {
		toSerialize["required_parameters"] = o.RequiredParameters
	}
	return json.Marshal(toSerialize)
}

type NullableExchangeLoginRequire struct {
	value *ExchangeLoginRequire
	isSet bool
}

func (v NullableExchangeLoginRequire) Get() *ExchangeLoginRequire {
	return v.value
}

func (v *NullableExchangeLoginRequire) Set(val *ExchangeLoginRequire) {
	v.value = val
	v.isSet = true
}

func (v NullableExchangeLoginRequire) IsSet() bool {
	return v.isSet
}

func (v *NullableExchangeLoginRequire) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableExchangeLoginRequire(val *ExchangeLoginRequire) *NullableExchangeLoginRequire {
	return &NullableExchangeLoginRequire{value: val, isSet: true}
}

func (v NullableExchangeLoginRequire) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableExchangeLoginRequire) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


