...
module EMS-ManagedCloudREST.API.Endpoints where
...
-- 
endpoints
  :: EMS-ManagedCloudRESTRequest Endpoints MimeNoContent [AccountEndpoint] MimeJSON
endpoints =
  _mkRequest "GET" ["/v1/endpoints"]
    `_hasAuthType` (P.Proxy :: P.Proxy AuthApiKeyAPIKeyHeader)
    `_hasAuthType` (P.Proxy :: P.Proxy AuthApiKeyAPIKeyQueryParam)

data Endpoints  

-- | /Optional Param/ "filter_exchange_id" - Exchange id
instance HasOptionalParam Endpoints FilterExchangeId where
  applyOptionalParam req (FilterExchangeId xs) =
    req `addQuery` toQueryColl MultiParamArray ("filter_exchange_id", Just xs)
-- | @application/json@
instance Produces Endpoints MimeJSON
