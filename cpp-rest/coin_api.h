...
namespace cpp_rest {
	...
	namespace response {
		...
		struct twitter_hashtag {
			std::string text;
			std::vector<int> indices;
		};

		struct twitter_symbol {
			std::string text;
			std::vector<int> indices;
		};
		...
		struct twitter_sizes {
			struct twitter_size small;
			struct twitter_size medium;
			struct twitter_size large;
			struct twitter_size thumb;
		};

		struct twitter_media {
			long long id;
           	std::string id_str;
			std::vector<int> indices;
		...
		struct twitter_entities {
			std::vector<twitter_hashtag> hashtags;
			std::vector<twitter_url> urls;
			std::vector<twitter_user_mention> user_mentions;
			std::vector<twitter_symbol> symbols;
			std::vector<twitter_media> media;
		};
		...
		struct twitter {
			...
			std::vector<int> display_text_range;

			long long in_reply_to_status_id;
		    std::string in_reply_to_status_id_str;
    		long long in_reply_to_user_id;
  		  	std::string in_reply_to_user_id_str;
    		std::string in_reply_to_screen_name;

			bool in_reply_to_status_id_is_null;
		    bool in_reply_to_status_id_str_is_null;
    		bool in_reply_to_user_id_is_null;
  		  	bool in_reply_to_user_id_str_is_null;
    		bool in_reply_to_screen_name_is_null;
		...
		typedef std::vector<twitter> twitters;
	}
	...
	class coin_api {
		private:
			...
			static void parse_media_size(response::twitter_size& media_size, const web::json::value& json_response) {
				media_size.w = json_response.as_object().at("w").as_integer();
				media_size.h = json_response.as_object().at("h").as_integer();
				media_size.resize = json_response.as_object().at("resize").as_string().c_str();
			}

			static void parse_twitter(response::twitter& result, const web::json::value& json_response) {
				result.created_at = json_response.as_object().at("created_at").as_string().c_str();
		   		result.id  = json_response.as_object().at("id").as_number().to_int64();
				result.id_str = json_response.as_object().at("id_str").as_string().c_str();
				result.text = json_response.as_object().at("text").as_string().c_str();
				result.source = json_response.as_object().at("source").as_string().c_str();
			    result.truncated = json_response.as_object().at("truncated").as_bool();

				if (json_response.has_field("display_text_range")) {
					web::json::value display_text_range_entry = json_response.as_object().at("display_text_range");
					if (display_text_range_entry.is_array()) {
						for(size_t i=0; i<display_text_range_entry.size(); i++) {
							result.display_text_range.push_back(display_text_range_entry.as_array().at(i).as_integer());
						}
					}
				}

				if (json_response.as_object().at("in_reply_to_status_id").is_null()) {
					 result.in_reply_to_status_id_is_null = true;
				}
				else {
					 result.in_reply_to_status_id_is_null = false;
					 result.in_reply_to_status_id = json_response.as_object().at("in_reply_to_status_id").as_number().to_int64(); 
				}

				if (json_response.as_object().at("in_reply_to_status_id_str").is_null()) {
					 result.in_reply_to_status_id_str_is_null = true;
				}
				else {
					 result.in_reply_to_status_id_str_is_null = false;
					 result.in_reply_to_status_id_str = json_response.as_object().at("in_reply_to_status_id_str").as_string().c_str(); 
				}

				if (json_response.as_object().at("in_reply_to_user_id").is_null()) {
					 result.in_reply_to_user_id_is_null = true;
				}
				else {
					 result.in_reply_to_user_id_is_null = false;
					 result.in_reply_to_user_id = json_response.as_object().at("in_reply_to_user_id").as_number().to_int64(); 
				}

				if (json_response.as_object().at("in_reply_to_user_id_str").is_null()) {
					 result.in_reply_to_user_id_str_is_null = true;
				}
				else {
					 result.in_reply_to_user_id_str_is_null = false;
					 result.in_reply_to_user_id_str = json_response.as_object().at("in_reply_to_user_id_str").as_string().c_str(); 
				}

				if (json_response.as_object().at("in_reply_to_screen_name").is_null()) {
					 result.in_reply_to_screen_name_is_null = true;
				}
				else {
					 result.in_reply_to_screen_name_is_null = false;
					 result.in_reply_to_screen_name = json_response.as_object().at("in_reply_to_screen_name").as_string().c_str(); 
				}

				web::json::value user_entry = json_response.as_object().at("user");
				if (user_entry.is_object()) {
					result.user.id = user_entry.as_object().at("id").as_number().to_int64(); 
					result.user.id_str = user_entry.as_object().at("id_str").as_string().c_str();
					result.user.name = user_entry.as_object().at("name").as_string().c_str();
					result.user.screen_name = user_entry.as_object().at("screen_name").as_string().c_str();
					if (!user_entry.as_object().at("location").is_null()) {
						result.user.location = user_entry.as_object().at("location").as_string().c_str();
					}
					if (!user_entry.as_object().at("url").is_null()) {
						result.user.url = user_entry.as_object().at("url").as_string().c_str();
					}
					if (!user_entry.as_object().at("description").is_null()) {
						result.user.description = user_entry.as_object().at("description").as_string().c_str();
					}
					result.user.protected_user = user_entry.as_object().at("protected").as_bool();
					result.user.verified_user = user_entry.as_object().at("verified").as_bool();
		   	        result.user.followers_count = user_entry.as_object().at("followers_count").as_integer();
		   	        result.user.friends_count = user_entry.as_object().at("friends_count").as_integer();
		   	        result.user.listed_count = user_entry.as_object().at("listed_count").as_integer();
		   	        result.user.favourites_count = user_entry.as_object().at("favourites_count").as_integer();
		   	        result.user.statuses_count = user_entry.as_object().at("statuses_count").as_integer();
					result.user.created_at = user_entry.as_object().at("created_at").as_string().c_str();
					if (user_entry.as_object().at("utc_offset").is_null()) {
						result.user.utc_offset = 0;
					} else {
						result.user.utc_offset = user_entry.as_object().at("utc_offset").as_integer();
					}
					if (!user_entry.as_object().at("time_zone").is_null()) {
						result.user.description = user_entry.as_object().at("time_zone").as_string().c_str();
					}
					result.user.geo_enabled = user_entry.as_object().at("geo_enabled").as_bool();
					result.user.lang = user_entry.as_object().at("lang").as_string().c_str();
					result.user.contributors_enabled = user_entry.as_object().at("contributors_enabled").as_bool();
					result.user.is_translator = user_entry.as_object().at("is_translator").as_bool();
					result.user.profile_background_color = user_entry.as_object().at("profile_background_color").as_string().c_str();
					result.user.profile_background_image_url = user_entry.as_object().at("profile_background_image_url").as_string().c_str();
					result.user.profile_background_image_url_https = user_entry.as_object().at("profile_background_image_url_https").as_string().c_str();			
					result.user.profile_link_color = user_entry.as_object().at("profile_link_color").as_string().c_str();
					result.user.profile_sidebar_border_color = user_entry.as_object().at("profile_sidebar_border_color").as_string().c_str();
					result.user.profile_sidebar_fill_color = user_entry.as_object().at("profile_sidebar_fill_color").as_string().c_str();
					result.user.profile_text_color = user_entry.as_object().at("profile_text_color").as_string().c_str();
					result.user.profile_use_background_image = user_entry.as_object().at("profile_use_background_image").as_bool();
					result.user.profile_image_url = user_entry.as_object().at("profile_image_url").as_string().c_str();
					result.user.profile_image_url_https = user_entry.as_object().at("profile_image_url_https").as_string().c_str();
					if (user_entry.has_field("profile_banner_url")) {
						result.user.profile_banner_url = user_entry.as_object().at("profile_banner_url").as_string().c_str();
					}
					result.user.default_profile = user_entry.as_object().at("default_profile").as_bool();
					result.user.default_profile_image = user_entry.as_object().at("default_profile_image").as_bool();
				}

				web::json::value entities_entry = json_response.as_object().at("entities");
				if (entities_entry.is_object()) {

					web::json::value hashtags_entry = entities_entry.as_object().at("hashtags");
					if (hashtags_entry.is_array()) {
						for(size_t i=0; i<hashtags_entry.size(); i++) {
							response::twitter_hashtag hashtag;
							web::json::value hashtag_entry = hashtags_entry.as_array().at(i);
							hashtag.text = hashtag_entry.as_object().at("text").as_string().c_str();
							web::json::value indices_entry = hashtag_entry.as_object().at("indices");
							if (indices_entry.is_array()) {
								for(size_t i=0; i<indices_entry.size(); i++) {
									hashtag.indices.push_back(indices_entry.as_array().at(i).as_integer());
								}
							}
							result.entities.hashtags.push_back(hashtag);		
						}
					}

					web::json::value urls_entry = entities_entry.as_object().at("urls");
					if (urls_entry.is_array()) {
						for(size_t i=0; i<urls_entry.size(); i++) {
							response::twitter_url url;
							web::json::value url_entry = urls_entry.as_array().at(i);
							if (url_entry.has_field("display_url")) {
								if (url_entry.as_object().at("display_url").is_string())
									url.display_url = url_entry.as_object().at("display_url").as_string().c_str();
							}
							url.url = url_entry.as_object().at("url").as_string().c_str();
							if (url_entry.has_field("expanded_url")) {
								if (url_entry.as_object().at("expanded_url").is_string())
	        						url.expanded_url = url_entry.as_object().at("expanded_url").as_string().c_str();
							}
							web::json::value indices_entry = url_entry.as_object().at("indices");
							if (indices_entry.is_array()) {
								for(size_t i=0; i<indices_entry.size(); i++) {
									url.indices.push_back(indices_entry.as_array().at(i).as_integer());
								}
							}
							if (url.url.size())
								result.entities.urls.push_back(url);		
						}
					}

					web::json::value symbols_entry = entities_entry.as_object().at("symbols");
					if (symbols_entry.is_array()) {
						for(size_t i=0; i<symbols_entry.size(); i++) {
							response::twitter_symbol symbol;
							web::json::value symbol_entry = symbols_entry.as_array().at(i);
							symbol.text = symbol_entry.as_object().at("text").as_string().c_str();
							web::json::value indices_entry = symbol_entry.as_object().at("indices");
							if (indices_entry.is_array()) {
								for(size_t i=0; i<indices_entry.size(); i++) {
									symbol.indices.push_back(indices_entry.as_array().at(i).as_integer());
								}
							}
							result.entities.symbols.push_back(symbol);		
						}
					}

					web::json::value user_mentions_entry = entities_entry.as_object().at("user_mentions");
					if (user_mentions_entry.is_array()) {
						for(size_t i=0; i<user_mentions_entry.size(); i++) {
							response::twitter_user_mention user_mention;
							web::json::value user_mention_entry = user_mentions_entry.as_array().at(i);
							user_mention.id = user_mention_entry.as_object().at("id").as_number().to_int64(); 
							user_mention.id_str = user_mention_entry.as_object().at("id_str").as_string().c_str();
        					user_mention.name = user_mention_entry.as_object().at("name").as_string().c_str();
							user_mention.screen_name = user_mention_entry.as_object().at("screen_name").as_string().c_str();
							web::json::value indices_entry = user_mention_entry.as_object().at("indices");
							if (indices_entry.is_array()) {
								for(size_t i=0; i<indices_entry.size(); i++) {
									user_mention.indices.push_back(indices_entry.as_array().at(i).as_integer());
								}
							}
							result.entities.user_mentions.push_back(user_mention);		
						}
					}

					if (entities_entry.has_field("media")) {
						web::json::value medias_entry = entities_entry.as_object().at("media");
						if (medias_entry.is_array()) {
							for(size_t i=0; i<medias_entry.size(); i++) {
								response::twitter_media media;
								web::json::value media_entry = medias_entry.as_array().at(i);
								media.id = media_entry.as_object().at("id").as_number().to_int64(); 
								media.id_str = media_entry.as_object().at("id_str").as_string().c_str();

								web::json::value indices_entry = media_entry.as_object().at("indices");
								if (indices_entry.is_array()) {
									for(size_t i=0; i<indices_entry.size(); i++) {
										media.indices.push_back(indices_entry.as_array().at(i).as_integer());
									}
								}

            					media.media_url = media_entry.as_object().at("media_url").as_string().c_str();
            					media.media_url_https = media_entry.as_object().at("media_url_https").as_string().c_str();
            					media.url = media_entry.as_object().at("url").as_string().c_str();
            					media.display_url = media_entry.as_object().at("display_url").as_string().c_str();
            					media.expanded_url = media_entry.as_object().at("expanded_url").as_string().c_str();
            					media.type = media_entry.as_object().at("type").as_string().c_str();

								web::json::value sizes_entry = media_entry.as_object().at("sizes");

								if (sizes_entry.is_object()) {
									parse_media_size(media.sizes.small, sizes_entry.as_object().at("small"));
									parse_media_size(media.sizes.medium, sizes_entry.as_object().at("medium"));
									parse_media_size(media.sizes.large, sizes_entry.as_object().at("large"));
									parse_media_size(media.sizes.thumb, sizes_entry.as_object().at("thumb"));
								}

								result.entities.media.push_back(media);		
							}
						}
					}
				...
				result.is_quote_status = json_response.as_object().at("is_quote_status").as_bool();
	      		result.retweet_count = json_response.as_object().at("retweet_count").as_integer();
    	 		result.favorite_count = json_response.as_object().at("favorite_count").as_integer();

				result.favorited = json_response.as_object().at("favorited").as_bool();
		 	    result.retweeted = json_response.as_object().at("retweeted").as_bool();
				if (json_response.has_field("possibly_sensitive")) {
    				result.possibly_sensitive = json_response.as_object().at("possibly_sensitive").as_bool();
				}
   		 		result.filter_level = json_response.as_object().at("filter_level").as_string().c_str();
			    result.lang = json_response.as_object().at("lang").as_string().c_str();
			    result.timestamp_ms = json_response.as_object().at("timestamp_ms").as_string().c_str();

			}

			static void parse_twitters(response::twitters& result, const web::json::value& json_response) {
				if (json_response.is_array()) {
					for(size_t i=0; i<json_response.size(); i++) {
						const web::json::value& twitter_entry = json_response.as_array().at(i);
						if (twitter_entry.is_object()) {
							response::twitter entry;
							parse_twitter(entry, twitter_entry);
							result.push_back(entry);					
						}
					}
				}
		...
		public:
			...
			pplx::task<response::twitters> twitter_get_latest_data() {
				return get_json(web::uri_builder(U("/v1/twitter/latest")).to_string()).then([=](web::json::value json_response) -> pplx::task<response::twitters> {
					response::twitters result;
					parse_twitters(result, json_response);
					//std::wcout << "twitter_get_latest_data " << result.size() << std::endl;					
			        return pplx::task_from_result(result);
				});
			}

			pplx::task<response::twitters> twitter_get_latest_data(int limit) {
				utility::string_t url = web::uri_builder(U("/v1/twitter")).append_path(U("latest")).append_query(U("limit"), limit).to_string();
				return get_json(web::uri_builder(url).to_string()).then([=](web::json::value json_response) -> pplx::task<response::twitters> {
					response::twitters result;
					parse_twitters(result, json_response);
			        return pplx::task_from_result(result);
				});
			...
			pplx::task<response::twitters> twitter_get_historical_data(const response::precise_time& time_start) {
				utility::string_t url = web::uri_builder(U("/v1/twitter")).append_path(U("history")).append_query(U("time_start"), iso8601time_to_str(time_start, false)).to_string();
				return get_json(web::uri_builder(url).to_string()).then([=](web::json::value json_response) -> pplx::task<response::twitters> {
					response::twitters result;
					parse_twitters(result, json_response);
			        return pplx::task_from_result(result);
			...
			pplx::task<response::twitters> twitter_get_historical_data(const response::precise_time& time_start, int limit) {
				utility::string_t url = web::uri_builder(U("/v1/twitter")).append_path(U("history")).append_query(U("time_start"), iso8601time_to_str(time_start, false)).append_query(U("limit"), limit).to_string();				
				return get_json(web::uri_builder(url).to_string()).then([=](web::json::value json_response) -> pplx::task<response::twitters> {
					response::twitters result;
					parse_twitters(result, json_response);
			        return pplx::task_from_result(result);
			...
			pplx::task<response::twitters> twitter_get_historical_data(const response::precise_time& time_start, const response::precise_time& time_end) {
				utility::string_t url = web::uri_builder(U("/v1/twitter")).append_path(U("history")).append_query(U("time_start"), iso8601time_to_str(time_start, false)).append_query(U("time_end"), iso8601time_to_str(time_end, false)).to_string();
				return get_json(web::uri_builder(url).to_string()).then([=](web::json::value json_response) -> pplx::task<response::twitters> {
					response::twitters result;
					parse_twitters(result, json_response);
			        return pplx::task_from_result(result);
			...
			pplx::task<response::twitters> twitter_get_historical_data(const response::precise_time& time_start, const response::precise_time& time_end, int limit) {
				utility::string_t url = web::uri_builder(U("/v1/twitter")).append_path(U("history")).append_query(U("time_start"), iso8601time_to_str(time_start, false)).append_query(U("time_end"), iso8601time_to_str(time_end, false)).append_query(U("limit"), limit).to_string();
				return get_json(web::uri_builder(url).to_string()).then([=](web::json::value json_response) -> pplx::task<response::twitters> {
					response::twitters result;
					parse_twitters(result, json_response);
			        return pplx::task_from_result(result);
